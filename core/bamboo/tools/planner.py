...
def _build_atlas_planner_prompt(schema_compact: str) -> str:
    ...
    return (
        ...
        "use panda_harvester_workers. route=FAST_PATH.\n"
        ...
        "- If the question asks about job timing or performance metrics from the "
        "historical OpenSearch index — stage-in time, stage-out time, wall-clock "
        "time, queue wait time, payload execution time, pilot setup time, or "
        "any pilottiming sub-field "
        "(e.g. 'average stage-in time at BNL', 'maximum queue time for failed jobs', "
        "'total wall-clock time at CERN today', 'minimum stagein time'): "
        "use atlas.job_timing. Pass site= if a site is mentioned. route=FAST_PATH.\n"
