...
def _installed_namespaces() -> set[str]:
    """Return the plugin namespaces present in this environment.

    Returns:
        Set of namespace prefixes from installed ``bamboo.tools`` entry points.
    """
    from bamboo.tools.loader import list_tool_entry_points

    return {
        str(ep["name"]).split(".", 1)[0]
        for ep in list_tool_entry_points()
        if "." in str(ep.get("name") or "")
    }


def _is_checkable(name: str) -> bool:
    """Return True if *name* belongs to a plugin installed in this environment.

    ``cgsim.sim_query`` is a real wire name wherever ``askcgsim`` is installed,
    and the core suite must not require every plugin.  This is a filter rather
    than a ``pytest.skip`` because skipping from inside a loop abandons the
    whole test — hitting an uninstalled plugin first would leave every ATLAS
    name unchecked while the run still looked green.

    Args:
        name: The literal under test.

    Returns:
        True when the name should be asserted against the wire surface.
    """
    namespace = name.split(".", 1)[0] if "." in name else ""
    return not namespace or namespace in _installed_namespaces()


def _assert_wire_names(names: list[str], context: str) -> None:
    """Assert every checkable name in *names* is a tool the server exposes.

    Membership, not idempotence.  ``canonical_tool_name`` returns an unknown
    name unchanged — deliberately, so ``_execute_one_tool`` can report it with
    the caller's spelling — which means a fixed-point check passes for a name
    that is not a tool at all.  ``pilot_code_query`` was exactly that, and a
    fixed-point guard did not catch it (verified by mutation).

    Args:
        names: Literals under test.
        context: Where the literals live, for the failure message.
    """
    wire = _wire_names()
    checked = 0
    for name in sorted(set(names)):
        if not _is_checkable(name):
            continue
        checked += 1
        assert name in wire, (
            f"{context} names {name!r}, which the MCP server does not expose. "
            f"Wire names: {sorted(wire)}"
        )
    assert checked, f"{context}: nothing was checked — the guard is inert"


class TestInternalLiteralsAreCanonical:
    """Every hard-coded tool name in core is a name the executor will see.

    These are the guards for the drift that produced this whole change. Each
    site below compares an exact string against ``called_tool_names``, which
    holds canonical wire names — a literal that is *nearly* right selects no
    specialist behaviour and falls through to a generic path. Nothing raises,
    so only a test of this shape catches it.
    """

    def test_synthesis_prompt_table_keys_are_canonical(self) -> None:
        """No entry in ``_TOOL_PROMPT_TABLE`` is unreachable.

        Two were: ``pilot_code_query``, which no tool has ever been called
        (``code_query`` is the advertised and wire name), so
        ``_SYSTEM_CODE_QUERY`` was never selected; and
        ``pilot_source_analysis``, the advertised rather than the wire name.
        Both degraded to the generic prompt, which reads as a mediocre answer
        rather than as a bug.
        """
        from bamboo.tools.bamboo_executor import _TOOL_PROMPT_TABLE

        _assert_wire_names(
            [name for name, _prompt in _TOOL_PROMPT_TABLE],
            "_TOOL_PROMPT_TABLE",
        )

    def test_doc_tool_sets_are_canonical(self) -> None:
        """RAG detection keys on these, so a stale name disables RAG framing."""
        from bamboo.tools.bamboo_executor import (
            _DEFAULT_DOC_TOOLS,
            _PLUGIN_DOC_TOOLS,
        )

        names = list(_DEFAULT_DOC_TOOLS)
        for tools in _PLUGIN_DOC_TOOLS.values():
            names.extend(tools)
        _assert_wire_names(names, "a doc-tool set")

    def test_fast_path_plans_name_tools_canonically(self) -> None:
        """Every ``ToolCall(tool="...")`` literal in bamboo_answer is canonical.

        Scanned from the source rather than by calling the routing functions,
        because the rules that build these plans are gated on question text,
        history and stored evidence; a test that exercised them would pin only
        the branches someone remembered to write a case for. The AST sees every
        literal, including in rules added later.

        This is the guard for the "three-file edit problem": adding a plugin
        tool means touching ``bamboo_answer``, ``planner`` and
        ``bamboo_executor``, and getting the name wrong in one of them fails
        silently.
        """
        import ast
        import pathlib

        import bamboo.tools.bamboo_answer as answer_mod

        source = pathlib.Path(answer_mod.__file__).read_text(encoding="utf-8")
        ...
        assert found, "no ToolCall(tool=...) literals found — has the AST shape changed?"
        _assert_wire_names(found, "a bamboo_answer fast-path plan")
