"""Tests for the panda_task_status tool (askpanda_epic plugin implementation).

All tests go through ``panda_task_status_tool`` and mock
``askpanda_epic._cache.cached_fetch_jsonish`` at the HTTP boundary.

The ePIC task_status implementation uses a single jobs endpoint
(``GET /jobs/?jeditaskid=<id>&json``) unlike the ATLAS implementation
which also fetches a separate task metadata endpoint.  There is no
``failed_pandaids`` field in the ePIC evidence dict.
...
from askpanda_epic import task_status as ts_mod
...
def _make_raw_task(
    ...
    return {
        ...
        "selectionsummary": [
            {
                "field": "taskname",
                "list": [{"value": f"epic_task_{task_id}", "count": 1}],
            },
            {
                "field": "jobstatus",
...
def test_task_status_success_json(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "askpanda_epic._cache.cached_fetch_jsonish",
...
def test_task_status_no_failed_pandaids_field(monkeypatch: pytest.MonkeyPatch) -> None:
    """ePIC evidence dict does not contain a 'failed_pandaids' field.

    The ePIC implementation uses a simpler schema than ATLAS — the flat
    failed_pandaids list is not included.
    """
    raw = _make_raw_task(task_id=1234, n_failed=2, n_finished=3)
    monkeypatch.setattr(
        "askpanda_epic._cache.cached_fetch_jsonish",
        lambda url, timeout=30: _ok_response(raw),
    )

    res = asyncio.run(ts_mod.panda_task_status_tool.call({"task_id": 1234}))
    ev = _unpack(res)["evidence"]
    assert "failed_pandaids" not in ev
...
def test_task_status_http_error_returns_error_evidence(
    ...
    monkeypatch.setattr("askpanda_epic._cache.cached_fetch_jsonish", _fail)
...
def test_task_status_url_uses_jobs_endpoint(
    ...
    Unlike the ATLAS implementation, ePIC uses only a single endpoint fetch
    (no separate task metadata endpoint).
    """
    ...
    monkeypatch.setattr("askpanda_epic._cache.cached_fetch_jsonish", _capture)
    monkeypatch.setenv("PANDA_BASE_URL", "https://bigpanda.cern.ch")

    asyncio.run(ts_mod.panda_task_status_tool.call({"task_id": 42}))

    # ePIC uses a single endpoint (jobs only), unlike ATLAS which fetches two.
...
def test_get_definition() -> None:
    ...
    d = ts_mod.panda_task_status_tool.get_definition()
    assert d["name"] == "panda_task_status"
    assert "task_id" in d["inputSchema"]["properties"]
    assert d["inputSchema"]["required"] == ["task_id"]
