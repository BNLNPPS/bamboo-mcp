...
import askpanda_atlas._core_dump_analyzer as acd  # noqa: E402
...
def test_hang_user_prompt_distinguishes_eventloop_success_from_job_success() -> None:
    """The synthesis prompt must state that EventLoop success is not PanDA job completion."""
    evidence = acd.CoreEvidence(mode="hang")
    evidence.diagnosis = {
        "available": True,
        "classification": "post-event-processing-shutdown-hang",
        "summary": "The EventLoop finished, but the payload hung during shutdown.",
        "job_completion": {
    ...
    prompt = acd.build_user_prompt(evidence)
    assert "SYNTHESIS CONSTRAINTS" in prompt
    assert "NOT proof of normal payload exit or PanDA job success" in prompt
    assert '"job_completed_normally": false' in prompt
    assert "Root cause established: False" in prompt
...
def test_llm_report_keeps_deterministic_hang_diagnosis_visible() -> None:
    """LLM mode must still show the authoritative normal-completion distinction."""
    evidence = acd.CoreEvidence(mode="hang")
    evidence.diagnosis = {
        "available": True,
        "classification": "post-event-processing-shutdown-hang",
        ...
        "summary": "EventLoop ended, but shutdown hung.",
        "job_completion": {
            "event_processing_completed": True,
            "payload_process_exited_normally": False,
            "job_completed_normally": False,
    ...
    analysis = {
        "verdict": "The payload hung during shutdown.",
        "classification": "hang",
        "confidence": "high",
        "confidence_reason": "Deterministic stack signature.",
    }
    report = acd.render_report(evidence, analysis)
    assert "DETERMINISTIC DIAGNOSIS" in report
    assert "EventLoop end : yes" in report
    assert "Payload exit  : no" in report
    assert "Normal job end: no" in report
...
def test_reconcile_llm_analysis_rejects_false_success_claim_for_looping_job() -> None:
    """A model cannot turn EventLoop completion into a successful overall job outcome."""
    evidence = acd.CoreEvidence(mode="hang")
    evidence.diagnosis = {
        "available": True,
        "classification": "post-event-processing-shutdown-hang",
        ...
        "root_cause_established": False,
        "summary": (
            "The EventLoop reported successful event processing, but the payload did not complete normally; "
            "it later hung during XRootD/XrdCl shutdown finalization."
        ),
    ...
    bad = {
        "verdict": "The job ran completely and successfully.",
        "classification": "undetermined",
        "confidence": "high",
        "likely_cause": "The job completed successfully and exited normally.",
        "busy_threads": "Shutdown threads were harmless.",
        "supporting_evidence": ["The payload job finished successfully."],
        "limitations": [],
        "next_steps": ["No action is needed because the job succeeded."],
        "explanation": "The payload completed normally and successfully; there was no hang.",
    }
    result = acd.reconcile_llm_analysis(evidence, bad)
    assert result["classification"] == "hang"
    assert result["confidence"] == "medium"
    assert "payload did not complete" in result["verdict"]
    rendered = json.dumps(result).lower()
    assert "job ran completely and successfully" not in rendered
    assert "job completed successfully" not in rendered
    assert "payload completed normally" not in rendered
    assert "job succeeded" not in rendered
    assert any("contradicted the deterministic hang outcome" in item for item in result["limitations"])
...
def test_analyze_with_llm_applies_deterministic_hang_guardrail(monkeypatch: pytest.MonkeyPatch) -> None:
    """The post-LLM reconciliation is applied before user-facing analysis is returned."""
    monkeypatch.setenv("ANTHROPIC_API_KEY", "sk-ant-test")
    reply = {
        "verdict": "The job completed successfully.",
        "classification": "undetermined",
        "confidence": "high",
        "likely_cause": "The process exited successfully.",
        "explanation": "The payload job finished successfully with no problem.",
    }
    captured: dict[str, Any] = {}
    _install_stub_sdk(monkeypatch, json.dumps(reply), captured)
    evidence = acd.CoreEvidence(mode="hang")
    evidence.diagnosis = {
        "available": True,
        "classification": "post-event-processing-remote-file-close-hang",
        "subtype": "remote-file-close",
        "confidence": "medium",
        "summary": "The EventLoop ended, but the payload hung while closing a remote file.",
    ...
    result = acd.analyze_with_llm(evidence, "claude-sonnet-4-6", 4000, backend="anthropic")
    assert result["classification"] == "hang"
    assert result["confidence"] == "medium"
    assert "payload did not complete" in result["verdict"]
    assert "SYNTHESIS CONSTRAINTS" in captured["messages"][0]["content"]
    assert "completed successfully" not in result["explanation"].lower()
...
def test_analyze_with_llm_tolerates_unparsable_reply(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    result = acd.analyze_with_llm(acd.CoreEvidence(), "claude-sonnet-4-6", 4000, backend="anthropic")
...
def test_analyze_with_llm_requires_api_key(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(RuntimeError, match="ANTHROPIC_API_KEY"):
        acd.analyze_with_llm(acd.CoreEvidence(), "claude-sonnet-4-6", 4000, backend="anthropic")
...
def test_analyze_with_llm_detail_logs_size_estimate(
    ...
    monkeypatch.setenv("ANTHROPIC_API_KEY", "sk-ant-test")
    _install_stub_sdk(monkeypatch, json.dumps({"verdict": "x"}), {})
    acd.analyze_with_llm(acd.CoreEvidence(), "claude-sonnet-4-6", 4000, progress=False,
                         detail=True, backend="anthropic")
    stderr = capsys.readouterr().err
...
def test_payload_completion_observations_place_hang_after_eventloop() -> None:
    ...
    assert any("EventLoop reported successful event processing" in item and "XRootD finalization" in item for item in obs)
    assert any("does not mean the payload process or PanDA job completed normally" in item for item in obs)
...
def test_structured_diagnosis_high_confidence_post_eventloop_shutdown_hang() -> None:
    ...
    assert diagnosis["signals"]["eventloop_status_success"] is True
    assert diagnosis["job_completion"]["event_processing_completed"] is True
    assert diagnosis["job_completion"]["payload_process_exited_normally"] is False
    assert diagnosis["job_completion"]["job_completed_normally"] is False
    assert "did not complete normally" in diagnosis["summary"]
...
def test_saved_looping_cases_share_family_but_have_distinct_subtypes() -> None:
    ...
    fixtures = [
        (fixture_dir / "core-analysis7.json", "poller-finalization"),
        (fixture_dir / "core-analysis9.json", "remote-file-close"),
    ]
...
# --------------------------------------------------------------------------- #
# Embedding contract and LLM backend selection
# --------------------------------------------------------------------------- #


def test_embedding_surface_is_importable() -> None:
    """The symbols Bamboo imports from this module must stay present and callable.

    Bamboo reuses these rather than reimplementing them, because
    ``reconcile_llm_analysis`` and the prompt builders together are the
    invariant that stops EventLoop completion markers being read as PanDA job
    success.  A rename here breaks the MCP tool at import time, so it is
    cheaper to fail in this suite.
    """
    for name in (
        "build_system_prompt",
        "build_user_prompt",
        ...
        assert hasattr(acd, name), f"{name} is part of the embedding surface"


def test_json_payload_declares_schema_and_tool_version(tmp_path: Path,
                                                       monkeypatch: pytest.MonkeyPatch) -> None:
    """The --json contract carries both versions, keyed independently.

    Bamboo keys on ``schema_version`` when reading the subprocess output. If it
    tracked ``tool_version`` instead, every analyzer release would look like a
    contract change.
    """
    out = tmp_path / "evidence.json"

    def _fake_collect(args: argparse.Namespace, progress: bool = True,
                      detail: bool = False) -> tuple[acd.CoreEvidence, str]:
        return acd.CoreEvidence(mode="hang"), "raw gdb output"

    monkeypatch.setattr(acd, "collect_evidence", _fake_collect)
    assert acd.main([str(tmp_path / "core.1"), "--no-llm", "--json", str(out), "-q"]) == 0

    payload = json.loads(out.read_text())
    assert payload["schema_version"] == acd.EVIDENCE_SCHEMA_VERSION
    assert payload["tool_version"] == acd.__version__
    assert payload["analysis"] is None
    assert "evidence" in payload


def test_resolve_llm_backend_explicit_names() -> None:
    """Explicit backend names bypass detection entirely."""
    assert acd.resolve_llm_backend("anthropic") == ("anthropic", acd._complete_via_anthropic)
    assert acd.resolve_llm_backend("bamboo") == ("bamboo", acd._complete_via_bamboo)


def test_resolve_llm_backend_rejects_unknown_name() -> None:
    """An unknown backend fails loudly rather than falling back silently."""
    with pytest.raises(RuntimeError, match="Unknown LLM backend"):
        acd.resolve_llm_backend("ollama-direct")


def test_resolve_llm_backend_auto_prefers_bamboo_when_importable() -> None:
    """auto uses Bamboo's configured provider when Bamboo is on the path.

    That is the point of the backend indirection: a run from a Bamboo checkout
    should use the provider that installation is already credentialed for
    rather than demanding a separate Anthropic key.
    """
    pytest.importorskip("bamboo.llm.factory")
    assert acd.resolve_llm_backend("auto")[0] == "bamboo"


def test_resolve_llm_backend_auto_falls_back_without_bamboo(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """auto degrades to the Anthropic SDK when Bamboo is not installed."""
    real_import = builtins.__import__

    def _blocked(name: str, *args: Any, **kwargs: Any) -> Any:
        if name.startswith("bamboo"):
            raise ImportError("bamboo is not installed")
        return real_import(name, *args, **kwargs)

    monkeypatch.setattr(builtins, "__import__", _blocked)
    assert acd.resolve_llm_backend("auto")[0] == "anthropic"


def test_bamboo_backend_refuses_inside_running_event_loop() -> None:
    """The synchronous bamboo backend must not be called from async code.

    asyncio.run() inside a live loop raises an opaque RuntimeError; this check
    replaces it with an actionable one naming the correct alternative.
    """
    pytest.importorskip("bamboo.llm.factory")

    async def _attempt() -> None:
        acd._complete_via_bamboo("system", "user", "some-model", 100)

    with pytest.raises(RuntimeError, match="running event loop"):
        asyncio.run(_attempt())


def test_bamboo_backend_reports_provider_and_model(monkeypatch: pytest.MonkeyPatch) -> None:
    """The bamboo backend records which provider and model actually answered.

    Without this the report cannot distinguish a Gemini answer from an Anthropic
    one, which matters when comparing analyses of the same core.
    """
    pytest.importorskip("bamboo.llm.factory")
    monkeypatch.setenv("LLM_DEFAULT_PROVIDER", "openai_compat")
    monkeypatch.setenv("LLM_DEFAULT_MODEL", "local-model")
    monkeypatch.delenv("LLM_REASONING_PROVIDER", raising=False)
    monkeypatch.delenv("LLM_REASONING_MODEL", raising=False)
    monkeypatch.delenv("LLM_PROFILES_JSON", raising=False)
    ...
    class _StubClient:
        ...
        async def generate(self, messages: Any, params: Any) -> Any:
            captured["messages"] = messages
            captured["max_tokens"] = params.max_tokens
            return types.SimpleNamespace(
                text=json.dumps({"verdict": "ok"}),
    ...
    import bamboo.llm.factory as factory_mod

    monkeypatch.setattr(factory_mod, "build_client", _StubClient)
    text, meta = acd._complete_via_bamboo("system prompt", "user prompt", None, 1234)

    assert json.loads(text) == {"verdict": "ok"}
    assert meta["backend"] == "bamboo"
    assert meta["provider"] == "openai_compat"
    assert meta["model"] == "local-model"
    assert meta["input_tokens"] == 11
    assert meta["output_tokens"] == 22
    assert captured["max_tokens"] == 1234
    assert captured["closed"] is True
    assert [m["role"] for m in captured["messages"]] == ["system", "user"]


def test_bamboo_backend_model_override_wins(monkeypatch: pytest.MonkeyPatch) -> None:
    """An explicit model overrides the profile's model but not its provider."""
    pytest.importorskip("bamboo.llm.factory")
    monkeypatch.setenv("LLM_DEFAULT_PROVIDER", "anthropic")
    monkeypatch.setenv("LLM_DEFAULT_MODEL", "profile-model")
    monkeypatch.delenv("LLM_PROFILES_JSON", raising=False)
    ...
    class _StubClient:
        ...
        async def generate(self, messages: Any, params: Any) -> Any:
            return types.SimpleNamespace(text="{}", usage=None)
    ...
    monkeypatch.setattr(factory_mod, "build_client", _StubClient)
    _text, meta = acd._complete_via_bamboo("s", "u", "requested-model", 100)

    assert captured["spec"].provider == "anthropic"
    assert meta["model"] == "requested-model"


def test_model_override_returns_none_when_unset(monkeypatch: pytest.MonkeyPatch) -> None:
    """With no model requested anywhere the backend keeps its own default.

    resolve_model() always yields a value, so using it here would force the
    analyzer's built-in Anthropic default over whatever model Bamboo's registry
    selected, silently defeating --llm-backend bamboo.
    """
    monkeypatch.delenv("CORE_ANALYSIS_MODEL", raising=False)
    monkeypatch.delenv("LLM_DEFAULT_MODEL", raising=False)
    assert acd.model_override(None) is None
    assert acd.model_override("explicit") == "explicit"

    monkeypatch.setenv("LLM_DEFAULT_MODEL", "from-bamboo-var")
    assert acd.model_override(None) == "from-bamboo-var"
    monkeypatch.setenv("CORE_ANALYSIS_MODEL", "from-core-var")
    assert acd.model_override(None) == "from-core-var"


def test_anthropic_backend_uses_builtin_default_without_override(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A None model reaching the Anthropic backend resolves to DEFAULT_MODEL."""
    monkeypatch.setenv("ANTHROPIC_API_KEY", "sk-ant-test")
    captured: dict[str, Any] = {}
    _install_stub_sdk(monkeypatch, json.dumps({"verdict": "x"}), captured)

    _text, meta = acd._complete_via_anthropic("s", "u", None, 100)
